...
export default function CertificateGenerator({ initialData, mode = 'create', isVoided: initialVoided = false, onSuccess }: CertificateGeneratorProps) {
  ...
  const renderCertificate = async () => {
    ...
    const leftMargin = 225 * scale;
    const maxTextWidth = width - leftMargin - 75 * scale;
        ...
        <div className="pt-12 pb-2 flex items-center gap-6 flex-wrap">
          ...
          {mode !== 'view' && (
            <button
              ...
              className="h-10 px-6 bg-slate-900 text-white rounded-lg text-[13px] font-semibold hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/20 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {isSubmitting ? (
                <>
