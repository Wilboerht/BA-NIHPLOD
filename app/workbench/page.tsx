    ...
    <div className="px-8 md:px-12 py-8 md:pt-10 md:pb-12 w-full max-w-7xl mx-auto flex flex-col flex-1 min-h-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <motion.div 
          ...
          className="notion-card flex items-center justify-between"
        >
          <div>
      ...
      <div className="flex-1 notion-card flex flex-col p-0 overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-white">
          ...
          <Link href="/workbench/complaints" className="text-[11px] font-bold text-blue-600 hover:underline uppercase tracking-widest">
            前往审核中台
          </Link>
        </div>
        <div className="flex-1 min-h-0 overflow-auto bg-slate-50/30 p-2">
           {isLoading ? (
             ...
             <div className="p-8 text-center flex flex-col items-center justify-center h-full">
                ...
                <p className="text-sm font-semibold text-slate-600">全部工单已清空</p>
                <p className="text-xs text-slate-400 mt-1">干得漂亮，没有积压的待处理举报。</p>
             </div>
           ) : (
             <div className="space-y-2">
               {recentComplaints.map(complaint => (
                 <div key={complaint.id} className="bg-white p-4 rounded-xl border border-slate-100 flex items-center justify-between hover:shadow-sm transition-shadow">
                   ...
                   <Link href={`/workbench/complaints?id=${complaint.id}`} className="shrink-0 p-2 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
