...
export default function VerifyPageContent() {
  ...
  useEffect(() => {
    ...
    const fetchCertificate = async () => {
      try {
        // 使用服务端 API 查询，与主页验证逻辑一致
        const response = await fetch("/api/certificates/verify", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ certNumber: decodeURIComponent(certId || "") })
        });

        const result = await response.json();

        if (!response.ok || !result.success) {
          setError(result.error || "查询失败，请稍后重试");
          setLoading(false);
          return;
        }
                ...
                <div className="space-y-3 bg-slate-50 p-4 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-900 font-semibold">{certificate.dealers?.company_name || certificate.company_name}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-900 font-semibold">
                        {certificate.dealers?.phone || certificate.phone || "未登记"}
                      </p>
                    </div>
                  </div>
                </div>
